						...
						<tbody>
							<c:forEach items="${map2.lists}" var="dto">
								<tr>
									...
									<td width="250"><a
										href="boardcontroller?action=getTipBoard&category=it&boardIndex=${dto.businessIdx}"><c:out
						...
						<ul class="pagination">
							...
							<c:if test="${map2.prevPage > 0}">
								<li class="page-item"><a class="page-link"
									href="boardcontroller?action=getBoard&page=${map2.prevPage}&category=tips&field=${map2.field }">&laquo;</a>
								</li>
							</c:if>
							<c:if test="${map2.pageCount <= 5}">
								<c:forEach begin="${map2.beginPage}"
									...
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="boardcontroller?action=getTipBoard&page=${page}&field=${map2.field}&category=it">${page}</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
							</c:if>
							<c:if test="${map2.pageCount > 5}">
								<c:forEach begin="${map2.beginPage}" end="${map2.endPage}"
									...
									<c:choose>
										...
										<c:otherwise>
											<li class="page-item"><a class="page-link"
												href="boardcontroller?action=getBoard&page=${page}&field=${map2.field}&category=it">${page}</a></li>
										</c:otherwise>
									</c:choose>
								</c:forEach>
							</c:if>
							...
							<c:if test="${map2.nextPage > 0}">
								<li class="page-item"><a class="page-link"
									href="boardcontroller?action=getTipBoard&page=${map2.nextPage}&field=${map2.field }&category=it">&raquo;</a>
								</li>
							</c:if>
						</ul>
					</div>
